...
// Path unit.
function pathLikeUnit(serverMethod, defaultStatusCode, httpMethods) {
  defaultStatusCode = defaultStatusCode || 200;
  ...
  return function unit(server) {
    ...
    var addMethodCallback = function (method, path, callback) {
      method = method.toLowerCase();
      if (!methodCallbacks[path]) {
        methodCallbacks[path] = {};
        addCallback(path, methodCallbacks[path]);
      }
      methodCallbacks[path][method] = callback;
    };
    httpMethods.forEach(function (method) {
      method = method.toLowerCase();
      server[method] = addMethodCallback.bind(null, method);
    });
    ...
    return function pathLayer (req, res, next) {
      ...
      if (cbindex >= 0) {
        getQueries(req, function(err) {
          ...
          var cb = callbacks[cbindex];
          var method = req.method.toLowerCase();
          if (cb instanceof Function) {
            cb(req, res);
          } else if (cb instanceof Object && cb[method] instanceof Function) {
            cb[method](req, res);
          } else {
